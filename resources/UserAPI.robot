...
Update the phone number and the email of the user
    ...
    ${StatusCode}=    Get From Dictionary    ${put_response.json()}    code
    ${StatusCode_as_integer}=    Convert To Integer    ${StatusCode}
    ...
    Dictionary Should Contain Item    ${put_response.json()}     code    ${StatusCode_as_integer}
    Dictionary Should Contain Item    ${put_response.json()}     message    ${userID}
...
UserAPI.Select the correct endpoint to delete the user previously
    ...
    Dictionary Should Contain Item    ${delete_resp.json()}     type    unknown
